    ...
    request.addEventListener("load", function () {
        ...
        for (let i = 0; i < news.news.length; i++) {
            ...
            request1.addEventListener("load", function () {
                ...
                let isPerm=false
                for (let j = 0; j < news.permissionsList.length; j++)
                    if (news.permissionsList[j] === 'GIVE')
                        isPerm=true
                        ...
                        <div class="sn-container">
                            ...
                            <div class="sn-content">
                                ...
                                <div class="block_addition">
                                    ${news.isAuth 
                                        ? `<form name="registerForm${news.news[i].id}">
                                            <input type="hidden" name="userId" value="${news.user.id}">
                                            <input type="hidden" name="newsId" value="${news.news[i].id}">
                                            ${ThisNews.isLike 
                                                ? `<button id="submitLike${news.news[i].id}" onclick="likePost(${news.news[i].id}, event)" class="btn btn-success" type="submit">Нравится&nbsp;&nbsp; ${news.news[i].likes}</button>` 
                                                : `<button id="submitLike${news.news[i].id}" onclick="likePost(${news.news[i].id}, event)" class="btn btn-danger" type="submit">Нравится&nbsp;&nbsp; ${news.news[i].likes}</button>`
                                            }
                                        </form>
                                        <form name="Watch${news.news[i].id}" style="margin-left: 500px;">
                                            <input type="hidden" name="userId" value="${news.user.id}">
                                            <input type="hidden" name="newsId" value="${news.news[i].id}">
                                            ${ThisNews.isSave 
                                                ? `<button id="WatchLater${news.news[i].id}" onclick="WatchLater(${news.news[i].id}, event)" class="btn btn-warning" type="submit">Убрать из избранного</button>` 
                                                : `<button id="WatchLater${news.news[i].id}" onclick="WatchLater(${news.news[i].id}, event)" class="btn btn-info" type="submit">Добавить в избранное</button>`
                                            }
                                        </form>`
